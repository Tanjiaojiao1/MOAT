...
class UDATrainer(Trainer):
    ...
    def train_one_epoch(self):
        ...
        for batch_s, batch_t in tqdm_epoch:
            ...
            x_target, _, _, _, _ = batch_t
            x_target = Variable(x_target).to(self.device)
            
            with torch.no_grad():
                pred_target, t_res, t_style = self.model(x_target)

            for num in range(4000):
                x_source, y_source, _, _, idx = batch_s
                x_source, y_source = Variable(x_source).to(self.device), Variable(y_source).to(self.device, dtype=torch.long)
            ...
            ########################
            # model output ->  list of:  1) pred; 2) feat from encoder's output
            #pred_target, re = pred_and_feat
            pred_target, res_t, t_style = self.model(x_target)
            ########################
