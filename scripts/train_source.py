...
from datasets.cityscapes_Dataset_ADA import City_Dataset, City_DataLoader, inv_preprocess, decode_labels
from datasets.gta5_Dataset_ADA import GTA5_DataLoader
from datasets.synthia_Dataset_ADA import SYNTHIA_DataLoader
...
datasets_path={
    'cityscapes': {
        'data_root_path': 'data/cityscapes',
        'list_path': 'datasets/Cityscapes',
    },

    'gta5': {
        'data_root_path': 'data/gta5',
        'list_path': 'datasets/GTA5',
    },

    'synthia': {
        'data_root_path': 'data/SYNTHIA',
...
for dataset in datasets_path.keys():
    for path in datasets_path[dataset].keys():
        datasets_path[dataset][path] = datasets_path[dataset][path]
...
class Trainer():
    ...
    def train(self):
        ...
        self.save_prototype(self.train_id+'prototype')
        ...
        with torch.no_grad():
            ...
            for x, y, _, _, id in tqdm_batch:
                ...
                if self.args.use_proto:
                    res = self.model(x)[1]  
                    output_proto = feat_prototype_distance(self.prototypes, res, self.args.num_classes)
                    weight_proto = get_prototype_weight(output_proto, 1) 
                    weight_proto = F.interpolate(weight_proto, size=pred.size()[2:], mode='bilinear', align_corners=True) 
                    pred = weight_proto * F.softmax(pred, dim=1)             
    ...
    def save_prototype(self, filename=None):
        filename = os.path.join(self.args.checkpoint_dir, filename)
...
def add_train_args(arg_parser):
    ...
    arg_parser.add_argument('--seed', default=12351, type=int, help='random seed')
    ...
    arg_parser.add_argument('--limits', type=float, default=None)
    arg_parser.add_argument('--imgaug', type=bool, default=True)
    arg_parser.add_argument('--adain', type=float, default=0.3)
    arg_parser.add_argument('--fda', type=tuple, default=("input/cityscapes", 0.01))
    arg_parser.add_argument('--styleaug', type=bool, default=False)
    arg_parser.add_argument('--autoaug', type=bool, default=True)
    arg_parser.add_argument('--use_proto', type=bool, default=False)
