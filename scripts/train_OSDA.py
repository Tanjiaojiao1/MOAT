...
class UDATrainer(Trainer):
    ...
    def __init__(self, args, cuda=None, train_id="None", logger=None):
        ...
        #self.prototypes = torch.load('prototypes_on_GTA5').to(self.device)
        #self.prototypes_num = torch.zeros([self.args.num_classes]).to(self.device) 
    ...
    def train_one_epoch(self):
        ...
        for i_iter in range(4000):
            ...
            with torch.no_grad():
                pred_s, s_res, _ = self.model(x_source, style=t_style)
                n, c, h, w = pred_s.shape
                p_s = F.softmax(pred_s, dim=1)
                logp_s = F.log_softmax(pred_s, dim=1)
                div = -(p_s * logp_s).sum() / (n * c * h * w)

                if (div / len(u)) > 0.0011:
                    self.logger.info("source image {}".format(idx))
                    break


            """
            with torch.no_grad():
                pred_s, res_s, _ = self.model(x_source, style=t_style)
        ...
        #with torch.no_grad():
        #    pred_tea, res_tea, _ = self.teacher_model(x_source)
        ...
        if self.args.use_proto:
            ...
            # update prototypes
            update_prototypes(res_s.detach(), y_source.detach(), self.prototypes, self.prototypes_num, self.args.num_classes, 0.9)

        y_source = torch.squeeze(y_source, 1)
        loss = self.loss(pred_source, y_source)
 
        #loss_cyc = self.consis_loss(F.log_softmax(pred_source, dim=1), F.softmax(pred_tea.detach(), dim=1))
        #loss = self.loss(weight_proto, y_source)

        if self.args.use_proto:
            loss_proto = 0.1 * self.loss(weight_proto, y_source)
            #print(loss_proto)
            loss_ = loss + loss_proto
        else:
            loss_ = loss

        loss_ = loss #+ 0.1 * loss_cyc
        ...
        """
        if self.args.use_proto:
            target_proto = feat_prototype_distance(self.prototypes, res_t, self.args.num_classes)
            target_proto_weight = get_prototype_weight(target_proto, 1)
            target_proto_weight = F.interpolate(target_proto_weight, size=pred_target.size()[2:], mode='bilinear', align_corners=True)
            pred_target = target_proto_weight * F.softmax(pred_target, dim=1) 

            argpred, argprob = torch.max(pred_target, dim=1)
            target_plabel = -1 * torch.ones(argpred.size()).to(self.device) 
            target_plabel = torch.where(argprob > 0.95, argpred, target_plabel)
            
            update_prototypes(res_t.detach(), target_plabel.detach(), self.prototypes, self.prototypes_num, self.args.num_classes, 0.9) 
        """ 
