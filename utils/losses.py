...
class EntropyLoss(nn.Module):
    def __init__(self, ignore_index=-1, num_classes=19):
    ...
    def forward(self, inputs):
        ...
        entropy = -torch.mul(inputs, torch.log2(inputs+1e-30)) / np.log2(self.num_classes)
        loss = torch.sum(entropy) / (n * h * w)
...
class IW_MaxSquareloss(nn.Module):
    ...
    def forward(self, pred, prob, label=None):
        ...
        probsq = torch.pow(prob, 2)
        #print("prob square", probsq.min())
        #print("prob square max", probsq.max())
        rank, _ = torch.sort(probsq)
        #print(probsq.shape)
        probsq = torch.where(probsq > 5e-1, probsq, torch.zeros_like(probsq).to(probsq.device, dtype=torch.float32))
        loss = -torch.sum((probsq * weights)[mask]) / (batch_size * self.num_class)
        
        return loss


class UncCELoss(nn.Module):
    def __init__(self, num_classes=19, gamma=0, eps=1e-7, size_average=True, one_hot=True, ignore=-1, weight=None):
